import { connectMetaWebhookResponse, N8N, IMetaAccount } from "../types/meta";
...
export const metaServices = {
   ...
   */
  async connectMetaWebhook(token: string): Promise<connectMetaWebhookResponse> {
// https://api.telegram.org/bot7616107595:AAHI1OYwd2TCvPX5IyGseg6ky1Myx5yzYfA/setWebhook?url=https://googlen8ndev.myftp.biz/webhook-test/api/assistant-hook&allowed_updates=message&max_connections=40
    ...
    try {
      const response = await api.get(`api/n8n/meta/createWebhook/${token}`);
      return response.data as connectMetaWebhookResponse;
    } catch (error) {
