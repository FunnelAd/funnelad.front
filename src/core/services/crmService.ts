import { ICustomer, CreateCustomerData, UpdateCustomerData } from "../types/crm";
...
export const crmServices = {
   ...
   */
  async getCustomersByIdBusiness(idBusiness: string): Promise<ICustomer[]> {
    try {
  ...
  async createCustomer(data: CreateCustomerData): Promise<ICustomer> {
    ...
    try {
      const response = await api.post("/api/crm/new-customer", data);
      return response.data as ICustomer;
    } catch (error) {
