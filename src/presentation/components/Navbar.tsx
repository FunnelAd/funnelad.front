  ...
  return (
    <nav className="w-full bg-[var(--fa-bg)] border-b border-[var(--fa-light)] px-4 py-2 flex items-center justify-between shadow z-50">
      ...
      <div className="flex items-center gap-3">
        <img src="/logo.png" alt="FunnelAd" className="h-8 w-8" />
        <span className="text-xl font-bold text-[var(--fa-white)] tracking-wide">FunnelAd</span>
      </div>
      {/* Selector de idioma y usuario */}
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-1">
          <span className="text-[var(--fa-white)] font-semibold mr-1">Idioma:</span>
          <select
            value={i18n.language}
          ...
          >
            {LANGUAGES.map(lang => (
              <option key={lang.code} value={lang.code} className="text-[var(--fa-bg)]">{lang.name}</option>
            ))}
          </select>
        </div>
        {/* Menú de usuario */}
        <Menu as="div" className="relative inline-block text-left">
          ...
          <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right bg-[var(--fa-white)] border border-[var(--fa-light)] divide-y divide-[var(--fa-light)] rounded-md shadow-lg focus:outline-none z-50">
            <div className="px-4 py-3">
              <p className="text-sm text-[var(--fa-bg)] font-semibold">{user?.name || 'Usuario'}</p>
              <p className="text-xs text-[var(--fa-gray)] truncate">{user?.email || 'usuario@email.com'}</p>
            </div>
            <div className="py-1">
              <Menu.Item>
                {({ active }) => (
                  <a
                    href="/profile"
                    className={`block px-4 py-2 text-sm rounded ${active ? 'bg-[var(--fa-light)] text-[var(--fa-bg)]' : 'text-[var(--fa-bg)]'}`}
                  >
                    Perfil
                  </a>
                )}
            ...
            <div className="py-1">
              <Menu.Item>
                {({ active }) => (
                  <button
                    className={`block w-full text-left px-4 py-2 text-sm rounded ${active ? 'bg-red-100 text-red-700' : 'text-[var(--fa-bg)]'}`}
                  >
                    Cerrar sesión
                  </button>
                )}
