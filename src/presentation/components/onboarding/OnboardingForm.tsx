...
const TEXT_CONFIG_OPTIONS = {
  useEmojis: [
    { id: 'none', name: 'Sin emojis', description: 'Comunicación formal sin emojis' },
    { id: 'few', name: 'Pocos emojis', description: 'Uso ocasional de emojis' },
    { id: 'moderate', name: 'Uso moderado', description: 'Balance entre formal y casual' },
    { id: 'many', name: 'Muchos emojis', description: 'Comunicación casual y expresiva' }
  ],
  textStyle: [
    { id: 'formal', name: 'Formal', description: 'Lenguaje profesional y estructurado' },
    { id: 'casual', name: 'Casual', description: 'Lenguaje amigable y cercano' },
    { id: 'technical', name: 'Técnico', description: 'Enfocado en detalles técnicos' },
    { id: 'sales', name: 'Ventas', description: 'Orientado a conversiones y ventas' }
  ],
  modelTemperature: [
    { id: '0.2', name: 'Baja', description: 'Respuestas más predecibles y conservadoras' },
    { id: '0.5', name: 'Media', description: 'Balance entre creatividad y precisión' },
    { id: '0.8', name: 'Alta', description: 'Respuestas más creativas y variadas' }
  ]
};
...
const VOICE_CONFIG_OPTIONS = {
  voiceType: [
    { id: 'male', name: 'Voz masculina', description: 'Tono grave y profesional' },
    { id: 'female', name: 'Voz femenina', description: 'Tono cálido y amigable' },
    { id: 'neutral', name: 'Voz neutral', description: 'Tono balanceado y versátil' }
  ],
  speechRate: [
  ...
  callHandling: [
    { id: 'inbound', name: 'Recibir llamadas', description: 'Solo responde llamadas entrantes' },
    { id: 'outbound', name: 'Realizar llamadas', description: 'Solo realiza llamadas salientes' },
    { id: 'both', name: 'Ambas direcciones', description: 'Recibe y realiza llamadas' }
  ]
};
  ...
  const validateStep = () => {
    ...
    if (currentStep === 1) {
      ...
      if (formData.textChannels.length === 0) {
        newErrors.textChannels = 'Debes seleccionar al menos un canal de comunicación';
      }
    }
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border ${errors.businessName ? 'border-red-500' : 'border-[#C9A14A]'} rounded-md focus:outline-none focus:ring-2 focus:ring-[#C9A14A] text-white bg-gray-700/80 backdrop-blur-sm`}
                  />
                ...
                >
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className={`w-full px-4 py-3 border ${errors.fullName ? 'border-red-500' : 'border-[#C9A14A]'} rounded-md focus:outline-none focus:ring-2 focus:ring-[#C9A14A] text-white bg-gray-700/80 backdrop-blur-sm`}
                  />
                ...
                >
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className={`w-full px-4 py-3 border ${errors.email ? 'border-red-500' : 'border-[#C9A14A]'} rounded-md focus:outline-none focus:ring-2 focus:ring-[#C9A14A] text-white bg-gray-700/80 backdrop-blur-sm`}
                  />
                ...
                >
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className={`w-full px-4 py-3 border ${errors.whatsapp ? 'border-red-500' : 'border-[#C9A14A]'} rounded-md focus:outline-none focus:ring-2 focus:ring-[#C9A14A] text-white bg-gray-700/80 backdrop-blur-sm`}
                  />
        ...
        return (
          ...
          >
            ...
            <p className="text-lg text-gray-300 mb-8 text-center max-w-2xl">
              Selecciona el tipo de asistente que mejor se adapte a tus necesidades. Puedes elegir uno o ambos.
            </p>
            
            {errors.agentType && (
              <p className="mb-4 text-red-500 font-medium">{errors.agentType}</p>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
              {AGENT_TYPES.map((agent) => (
                ...
                >
                  ...
                  <p className="text-gray-300 text-center">{agent.description}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
      ...
      case 3: // Canales de texto
        return (
          ...
          >
            ...
            {errors.textChannels && (
              <p className="mb-4 text-red-500 font-medium">{errors.textChannels}</p>
            )}
        ...
        return (
          ...
          >
            ...
            <div className="w-full max-w-4xl space-y-4 md:space-y-6">
              {/* Uso de emojis */}
              <div>
                <h3 className="text-lg font-semibold text-white mb-2 md:mb-3">Uso de emojis</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                  {TEXT_CONFIG_OPTIONS.useEmojis.map((option) => (
                    <motion.div
                      ...
                      whileTap={{ scale: 0.97 }}
                      onClick={() => handleTextConfigChange('useEmojis', option.id)}
                    ...
                    >
                      ...
                      <p className="text-xs mt-1 opacity-80 hidden md:block">{option.description}</p>
                      {formData.textConfig.useEmojis === option.id && (
              ...
              {/* Estilo de texto */}
              <div>
                <h3 className="text-lg font-semibold text-white mb-2 md:mb-3">Estilo de comunicación</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                  {TEXT_CONFIG_OPTIONS.textStyle.map((option) => (
                    <motion.div
                      ...
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                      onClick={() => handleTextConfigChange('textStyle', option.id)}
                      className={`p-3 rounded-lg cursor-pointer transition-all duration-300 ${
              ...
              <div>
                <h3 className="text-lg font-semibold text-white mb-2 md:mb-3">Creatividad de respuestas</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-3">
                  {TEXT_CONFIG_OPTIONS.modelTemperature.map((option) => (
                    <motion.div
                      ...
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                      onClick={() => handleTextConfigChange('modelTemperature', option.id)}
                      className={`p-3 rounded-lg cursor-pointer transition-all duration-300 ${
        ...
        return (
          ...
          >
            ...
            <div className="w-full max-w-4xl space-y-8">
              {/* Tipo de voz */}
              <div>
                <h3 className="text-xl font-semibold text-white mb-4">Tipo de voz</h3>
                <div className="relative w-full max-w-md">
                  <select
                    value={formData.voiceConfig.voiceId || ''}
                    onChange={(e) => handleVoiceConfigChange('voiceId', e.target.value)}
                  ...
                  >
                    ...
                    {AVAILABLE_VOICES.map((voice) => (
                      <option key={voice.id} value={voice.id}>
                        {voice.name} ({voice.voice_type === 'masculine' ? 'Masculina' : voice.voice_type === 'feminine' ? 'Femenina' : 'Neutral'})
                      </option>
                    ))}
                  </select>
                  <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <svg className="w-5 h-5 text-[#C9A14A]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Velocidad de habla */}
              <div>
                <h3 className="text-xl font-semibold text-white mb-4">Velocidad de habla</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {VOICE_CONFIG_OPTIONS.speechRate.map((option) => (
                    <motion.div
                      ...
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => handleVoiceConfigChange('speechRate', option.id)}
              ...
              {/* Manejo de llamadas */}
              <div>
                <h3 className="text-xl font-semibold text-white mb-4">Manejo de llamadas</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {VOICE_CONFIG_OPTIONS.callHandling.map((option) => (
                    <motion.div
                      ...
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => handleVoiceConfigChange('callHandling', option.id)}
                      className={`p-4 rounded-lg cursor-pointer transition-all duration-300 ${
              ...
              <div>
                <h3 className="text-xl font-semibold text-white mb-4">Temperatura del agente</h3>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => handleVoiceConfigChange('temperature', e.target.value)}
          ...
          >
            <div className="text-center">
              ...
              <div className="text-lg text-gray-300 mb-8 max-w-2xl mx-auto">
                <p className="mb-4">
                  Hemos configurado tus agentes para <span className="text-[#C9A14A] font-semibold">{formData.businessName}</span>.
                </p>
                
                <ul className="space-y-2 text-left mx-auto max-w-md">
                  {formData.agentTypes.includes('text') && (
                    <li className="flex items-center">
                      <span className="mr-2">•</span> Agente de texto para responder mensajes
                    </li>
                  )}
                  {formData.agentTypes.includes('voice') && (
                    <li className="flex items-center">
                      <span className="mr-2">•</span> Agente de voz para llamadas
                    </li>
                  )}
                </ul>
                
                <p className="mt-6">
                  Te enviaremos un correo a <span className="text-[#C9A14A] font-semibold">{formData.email}</span> con los detalles.
                </p>
              </div>
    ...
    return (
      <div className="flex justify-center space-x-2 mb-8">
        {STEPS.map((step, index) => (
          <motion.div
            key={step.id}
            className={`h-2 rounded-full ${currentStep === index ? 'bg-[#C9A14A] w-8' : 'bg-gray-600 w-4'} transition-all duration-300`}
            initial={false}
            animate={{ 
              width: currentStep === index ? 32 : 16,
              backgroundColor: currentStep === index ? '#C9A14A' : currentStep > index ? '#8B7535' : '#4B5563'
            }}
          />
        ))}
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#001524] to-[#003049] text-white overflow-x-hidden">
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-7xl">
            ...
            {getVisibleSteps().map((step, index) => (
              <div
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  index <= getVisibleStepIndex() ? 'bg-[#C9A14A]' : 'bg-gray-600'
        ...
        <div className="relative bg-[#001F2E]/80 backdrop-blur-sm rounded-2xl p-6 md:p-8 mb-6 overflow-x-hidden" style={{ minHeight: '400px', maxHeight: 'calc(100vh - 250px)', overflowY: 'auto' }}>
