...
export default function CreateAssistantModal({
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      if (isEditing && assistant) {
        ...
        const modelFromPrompt = predefinedModels.find(m => m.prompt === assistant.prompt);
  ...
  const handleModelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedModel = predefinedModels.find((model) => model.id === modelId);
  ...
  const filteredVoices =
    selectedProvider === "all"
      ...
      : availableVoices.filter(
          (voice) => voice.provider.toLowerCase() === selectedProvider.toLowerCase()
        );
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("assistant_name")}</label>
                    <input
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("Mensaje De Bienvenida")}</label>
                    <input
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900 placeholder-gray-400"
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("response_time")}</label>
                    <input
                      ...
                      onChange={(e) => handleChange("timeResponse", Number(e.target.value))}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Proporción de respuesta</label>
                    <div className="space-y-2">
                      <input
                        type="range" min={0} max={100} value={form.assistensResponseP}
                        onChange={(e) => handleChange("assistensResponseP", Number(e.target.value))}
                        className="w-full accent-blue-500"
                      />
                      <div className="flex justify-between text-xs text-gray-600">
                        <span>{t("text")}</span>
                        <span className="font-semibold text-blue-600">{form.assistensResponseP}% {t("text")} / {100 - form.assistensResponseP}% {t("audio")}</span>
                        <span>{t("audio")}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="mb-6">
                  <label className="block text-sm font-medium text-gray-700 mb-3">Tipo de entrenamiento</label>
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="trainingType" value="predefined" checked={form.trainingType === "predefined"} onChange={(e) => handleChange("trainingType", e.target.value)} className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"/>
                      <span className="text-sm text-gray-700 font-medium">Modelos Predeterminados</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="trainingType" value="custom" checked={form.trainingType === "custom"} onChange={(e) => handleChange("trainingType", e.target.value)} className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"/>
                      <span className="text-sm text-gray-700 font-medium">Entrenamiento Personalizado</span>
                    </label>
                  </div>
                </div>
                {form.trainingType === "predefined" && (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Seleccionar modelo</label>
                      <select
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900"
                        value={form.selectedModel}
                        onChange={handleModelChange}
                      >
                        <option value="">Selecciona un modelo predeterminado</option>
                        {predefinedModels.map((model) => (<option key={model.id} value={model.id}>{model.name}</option>))}
                      </select>
                    </div>
                    {selectedModelData && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <h4 className="font-medium text-blue-900 mb-2">{selectedModelData.name}</h4>
                        <p className="text-sm text-blue-700 mb-3">{selectedModelData.description}</p>
                        <div className="bg-white rounded p-3 border border-blue-200">
                          <p className="text-xs font-medium text-gray-600 mb-1">Vista previa del prompt:</p>
                          <p className="text-sm text-gray-800 line-clamp-3">{selectedModelData.prompt}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {form.trainingType === "custom" && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Instrucciones y personalidad del asistente</label>
                    <textarea
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                      <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" /></svg>
                      {t("text")}
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                          <div className="flex items-center gap-3">
                              <input type="checkbox" id="emotesUse" checked={form.emotesUse} onChange={(e) => handleChange("emotesUse", e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                              <label htmlFor="emotesUse" className="text-sm text-gray-700 font-medium">{t("use_emojis")}</label>
                          </div>
                          <div className="flex items-center gap-3">
                              <input type="checkbox" id="stylesUse" checked={form.stylesUse} onChange={(e) => handleChange("stylesUse", e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                              <label htmlFor="stylesUse" className="text-sm text-gray-700 font-medium">{t("use_styles")}</label>
                          </div>
                      </div>
                  </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {["all", "OpenAI", "ElevenLabs", "Azure"].map((provider) => (
                          <button
                            key={provider}
                            onClick={() => setSelectedProvider(provider)}
                            className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${selectedProvider === provider ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
                          >
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("audio_voice")}</label>
                    {loadingVoices ? (
                      <div className="flex items-center justify-center p-4 border border-gray-300 rounded-lg bg-white">
                        <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500"></div>
                        <span className="ml-2 text-gray-600">Cargando voces...</span>
                      </div>
                    ) : (
                      <VoiceSearchCombobox
                        voices={filteredVoices}
                        selectedValue={form.voice}
                        onSelect={(selectedVoice) => handleChange("voice", selectedVoice)}
                      />
                    )}
                  </div>
                  {form.voice?.id ? (
                    <div className="md:col-span-2 bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <h4 className="font-medium text-blue-900 mb-2">Voz seleccionada</h4>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                           <div className={`w-3 h-3 rounded-full ${form.voice.gender === "male" ? "bg-blue-400" : form.voice.gender === "female" ? "bg-pink-400" : "bg-gray-400"}`} />
                          <span className="font-medium text-blue-900">{form.voice.name}</span>
                          <span className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-full">{form.voice.provider}</span>
                        </div>
                        <button
                        ...
                        >
                          {testingVoice === form.voice.id ? 'Probando...' : 'Probar voz'}
                        </button>
                      </div>
                    </div>
                  ) : null}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("audio_count")}</label>
                    <input
                      type="number" min={0} max={10}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900"
                      value={form.amountAudio}
                      onChange={(e) => handleChange("amountAudio", Number(e.target.value))}
                    />
                    <p className="mt-1 text-xs text-gray-500">Número máximo de mensajes de audio por conversación</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Configuraciones de audio</label>
                    <div className="space-y-2">
                      <label className="flex items-center gap-2 text-sm">
                        <input type="checkbox" checked={form.voiceResponse || false} onChange={(e) => handleChange("voiceResponse", e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                        Responder audios con audio
                      </label>
                      <label className="flex items-center gap-2 text-sm">
                        <input type="checkbox" checked={form.autoTranscribe || false} onChange={(e) => handleChange("autoTranscribe", e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
          ...
          {activeTab === "integrations" && (
            <div className="bg-gray-50 rounded-xl p-6">
              ...
              <h5 className="text-md font-semibold text-gray-600 mb-4 flex items-center gap-3 border-b pb-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16"><path d="M11.386 1.026A.5.5 0 0 1 12 1.5v13a.5.5 0 0 1-.614.474l-4.524-2.093-4.524 2.093A.5.5 0 0 1 1.5 14.5v-13a.5.5 0 0 1 .614-.474l4.524 2.093zM2 2.252v11.13l3.927-1.825a.5.5 0 0 1 .446 0l4.244 1.964V2.252l-3.927 1.825a.5.5 0 0 1-.446 0z"/></svg>
                  Meta
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("whatsapp_number")}</label>
                    <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.idPhoneNumber} onChange={(e) => handleChange("idPhoneNumber", e.target.value)} placeholder="ID del número de WhatsApp"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("whatsapp_business_id")}</label>
                    <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.idWppBusinessAccount} onChange={(e) => handleChange("idWppBusinessAccount", e.target.value)} placeholder="ID de la cuenta de negocio"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("meta_app_id")}</label>
                    <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.idMetaApp} onChange={(e) => handleChange("idMetaApp", e.target.value)} placeholder="ID de la aplicación Meta"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t("meta_token")}</label>
                    <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.tokenMetaPermanent} onChange={(e) => handleChange("tokenMetaPermanent", e.target.value)} placeholder="Token permanente de Meta"/>
                  </div>
              </div>

              <h5 className="text-md font-semibold text-gray-600 mb-4 flex items-center gap-3 border-b pb-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.287 5.906c-.778.324-2.334.994-4.666 2.01-.378.15-.577.298-.595.442-.03.243.275.339.69.47l.175.055c.408.133.958.288 1.243.294.26.006.549-.1.868-.32 2.179-1.471 3.304-2.214 3.374-2.23.05-.012.12-.026.166.016.047.041.042.12.037.141-.03.129-1.227 1.241-1.846 1.817-.193.18-.33.307-.358.336a8 8 0 0 1-.188.186c-.38.366-.664.64.015 1.088.327.216.589.393.85.571.284.194.568.387.936.629.093.06.183.125.27.187.331.236.63.448.997.414.214-.02.435-.22.547-.82.265-1.417.786-4.486.906-5.16.123-1.475-.517-2.043-1.177-2.152-1.224-.21-4.224.45-5.5.94z"/></svg>
                  Telegram
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
                <div className="flex items-end gap-2">
                    <div className="flex-grow">
                      <label className="block text-sm font-medium text-gray-700 mb-2">{t("token_telegram")}</label>
                      <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.tokenTelegram} onChange={(e) => handleChange("tokenTelegram", e.target.value)} placeholder="123456:ABC-DEF..."/>
                    </div>
                    <button onClick={onConnectTelegram} className="px-4 py-2 h-10 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 whitespace-nowrap">{t("connnect")}</button>
                </div>
              </div>
              
              <h5 className="text-md font-semibold text-gray-600 mb-4 flex items-center gap-3 border-b pb-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16"><path d="M4.182 1.223a.5.5 0 0 1 .536.078l.25.292c.16.188.243.44.195.688l-.226 1.13a.5.5 0 0 1-.588.42l-1.147-.23a.5.5 0 0 0-.533.262l-.25.5a.5.5 0 0 0 .042.585l1.002 1.114a.5.5 0 0 0 .633.102l1.1-.44a.5.5 0 0 1 .6-.053l.25.292a.5.5 0 0 0 .693-.117l.95-1.329a.5.5 0 0 0-.053-.6l-1.1-.917a.5.5 0 0 1-.078-.536l.25-.417a.5.5 0 0 0-.117-.693l-1.05-.7a.5.5 0 0 0-.585.042l-.5.25a.5.5 0 0 1-.65-.106L5.35 1.429a.5.5 0 0 0-.262-.533l-1.042-.521a.5.5 0 0 1-.42.588l1.13.226c.248.048.5-.034.688-.195zm7.364 1.342a.5.5 0 0 1 .6.053l1.1.917c.16.133.19.358.078.536l-.25.417a.5.5 0 0 0 .117.693l1.05.7a.5.5 0 0 0 .585-.042l.5-.25a.5.5 0 0 1 .65.106l.586.976a.5.5 0 0 0 .262.533l1.042.521a.5.5 0 0 1-.42-.588l-1.13-.226a.5.5 0 0 0-.688.195l-.25.292a.5.5 0 0 1-.536.078l-1.147-.573a.5.5 0 0 0-.533.262l-.25.5a.5.5 0 0 0 .042.585l1.002 1.114a.5.5 0 0 0 .633.102l1.1-.44a.5.5 0 0 1 .6-.053l.25.292a.5.5 0 0 0 .693-.117l.95-1.329a.5.5 0 0 0-.053-.6l-1.1-.917a.5.5 0 0 1-.078-.536l.25-.417a.5.5 0 0 0-.117-.693l-1.05-.7a.5.5 0 0 0-.585.042l-.5.25a.5.5 0 0 1-.65-.106l-.586-.976a.5.5 0 0 0-.262-.533l-1.042-.521a.5.5 0 0 1 .588.42l.226 1.13a.5.5 0 0 0 .42.588zM4.62 8.153a.5.5 0 0 1 .536.078l.25.292c.16.188.243.44.195.688l-.226 1.13a.5.5 0 0 1-.588.42l-1.147-.23a.5.5 0 0 0-.533.262l-.25.5a.5.5 0 0 0 .042.585l1.002 1.114a.5.5 0 0 0 .633.102l1.1-.44a.5.5 0 0 1 .6-.053l.25.292a.5.5 0 0 0 .693-.117l.95-1.329a.5.5 0 0 0-.053-.6l-1.1-.917a.5.5 0 0 1-.078-.536l.25-.417a.5.5 0 0 0-.117-.693l-1.05-.7a.5.5 0 0 0-.585.042l-.5.25a.5.5 0 0 1-.65-.106l-.586-.976a.5.5 0 0 0-.262-.533l-1.042-.521a.5.5 0 0 1-.42.588l1.13.226c.248.048.5-.034.688-.195z"/></svg>
                Configuración de Webhook
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">URL del Webhook</label>
                      <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.webhook} onChange={(e) => handleChange("webhook", e.target.value)} placeholder={t("webhook_url")}/>
                  </div>
                  <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Token del Webhook</label>
                      <input className="w-full px-3 py-2 border border-gray-300 rounded-lg" value={form.tokenWebhook} onChange={(e) => handleChange("tokenWebhook", e.target.value)} placeholder={t("webhook_token")}/>
                  </div>
              </div>
            </div>
          )}
